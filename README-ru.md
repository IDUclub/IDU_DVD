...
- LLM-эндпоинт для разметки, мерджа, тегов и определения версии (Ollama или
  OpenAI-совместимый сервер).
- Сервис giga-vectorizer с `Giga-Embeddings-instruct` (дефолт, размерность 2048).
  Ollama `bge-m3` остаётся резервным 1024-мерным провайдером.
...
### 2. Модели и векторизатор

Запустите giga-vectorizer с `Giga-Embeddings-instruct`. LLM — по вашему выбору
(см. `docs/ru/configuration.md`), например через Ollama:

```
ollama pull qwen2.5:7b-instruct
```

Для CPU-резерва задайте `DVD_EMBEDDINGS_PROVIDER=ollama` и загрузите `bge-m3`.
...
У каждой настройки есть разумный дефолт в коде (`src/common/config/app_config.py`), так что
приложение запускается и вовсе без `.env`; пример покрывает сетевые связи сервиса (адреса
Ollama, Qdrant, Redis, Kafka) — то, что обычно переопределяют. Полный справочник переменных — `.env.full.example` и `docs/ru/configuration.md`.
Переменные имеют префикс `DVD_`. Дефолтное векторное пространство — Giga/2048; при старте сервис
опрашивает активный векторизатор и использует `DVD_VECTOR_SIZE` только как резерв, если опрос
недоступен.
...
(СП 19.13330.2019) с локальной Ollama. Поддерживаются OCR-free текстовые форматы из коробки
(`.docx`, `.txt`, `.md`, `.html` — настраивается через `DVD_ALLOWED_EXTENSIONS`); скан-PDF/OCR
планируется.
