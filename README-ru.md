...
Скопируйте минимальный пример и при необходимости поправьте адреса и модели:
...
приложение запускается и вовсе без `.env`; минимальный пример покрывает то, что обычно
переопределяют. Полный справочник переменных — `.env.full.example` и `docs/ru/configuration.md`.
Переменные имеют префикс `DVD_`. Размерность `DVD_VECTOR_SIZE` должна совпадать с размерностью
эмбеддинг-модели (`bge-m3` = 1024).
