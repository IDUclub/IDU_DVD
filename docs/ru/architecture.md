...
векторизация) выполняется не в том запросе, который принёс документ: оригинал уходит в MinIO,
задание — в устойчивую очередь в Redis, а разбирает её пул фоновых воркеров, поэтому загрузка
переживает и отключившегося клиента, и перезапуск сервиса. Статусы задач, сама очередь и реестр
документов ведутся в Redis;
...
| Компонент | Роль |
|-----------|------|
| FastAPI | HTTP API; воркеры загрузки работают фоновыми задачами его lifespan |
| Qdrant | векторная база; отдельная коллекция под каждое векторное пространство (namespacing), payload-индексы |
| Redis | очередь загрузки (ожидание / в работе / dead-letter), статусы задач парсинга, реестр документов и версий (неймспейсится по коллекции), outbox Kafka-событий. Его устойчивость — это устойчивость очереди, поэтому запускать с включённым AOF |
| Бэкенд LLM | разметка, мердж, теги, «голова» документа, ссылки. По умолчанию — любой **OpenAI-совместимый** сервер `/v1/chat/completions` (vLLM, LM Studio, llama.cpp, OpenAI); нативная **Ollama** `/api/chat` — для локальной разработки. Выбор через `DVD_LLM_PROVIDER`. Не опционален: если он недоступен, документ невозможно структурировать, и загрузка падает, а не индексирует его безымянным |
...
фоновую обработку потокобезопасной — и позволяет держать больше одного воркера загрузки.

У параллелизма загрузки ровно один ограничитель — число воркеров (`DVD_INGEST_CONCURRENCY`).
Документ обрабатывается одним воркером от начала до конца, а всё остальное ждёт записью в Redis,
а не заблокированным потоком, поэтому очередь из сотни документов ничего не стоит процессу.
