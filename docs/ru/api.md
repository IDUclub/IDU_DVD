...
На **каждый** документ ставится своя фоновая задача (`202`); прогресс — через
...
Индексация выполняется фоновой задачей внутри процесса приложения и **не возобновляется** после
перезапуска сервиса. Поэтому при старте все задачи, оставшиеся в статусе `queued`/`processing`,
переводятся в `error` с пояснением «прервана перезапуском сервиса» (в логе —
`orphaned_jobs_aborted`), а временные файлы в `DVD_UPLOAD_DIR` удаляются. Прерванный документ нужно
загрузить заново; исходник в MinIO и записи в Qdrant/Redis при этом не затрагиваются.
